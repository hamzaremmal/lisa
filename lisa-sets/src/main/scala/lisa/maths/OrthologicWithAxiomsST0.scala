...
object OrthologicWithAxiomsST0 extends lisa.Main:
  ...
  def neg2(t: Term): Term = app(OrthologicWithAxiomsST0.neg2, x)
  extension (left: Term)
    def leq(right: Term): Formula = in(pair(left, right), OrthologicWithAxiomsST0.leq)
    def meet(right: Term): Term = app(OrthologicWithAxiomsST0.meet, pair(left, right))
    def join(right: Term): Term = app(OrthologicWithAxiomsST0.join, pair(left, right))
