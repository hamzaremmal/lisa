...
object RestateWithAxioms extends ProofTactic:
  ...
  def apply(using lib: OrthologicWithAxiomsLibrary, proof: lib.Proof)(bot: Sequent): proof.ProofTacticJudgement =
    import lib.{isO, in, U, <=, unorderedPair}
    val leq = <=
    ...
    if bot.right.size != 1 then
      ...
      case Leq(left, right) =>
        ...
        withParameters(termsInU, axioms)(L(left), R(right))
  ...
  def withParameters(using lib: OrthologicWithAxiomsLibrary, proof: lib.Proof)
    ...
    import lib.{isO, inU, <=, unorderedPair, in, assume, have, andThen, lastStep, `0`, `1`, U, ∈, zero2term, one2term, x, y, z, app, not, n ,u}
    val (leq, meet, join) = (<=, n, u)
    ...
    def proveInU(using proof: lib.Proof)(t: Term): proof.ProofTacticJudgement =
      t match
        case x1 if termsInU contains x1 =>
          TacticSubproof:
            have(premises |- x1 ∈ U) by RewriteTrue
        case `0` =>
          TacticSubproof:
            have(premises |- 0 ∈ U) by Tautology.from(lib.zeroInOrtholattice) // IMPROVE
        case `1` =>
          TacticSubproof:
            have(premises |- 1 ∈ U) by Tautology.from(lib.oneInOrtholattice)
        case Not(x1) =>
          proveInU(x1) andThen2 { lastStep =>
            have(premises |- !x1 ∈ U) by Cut(lastStep, lib.negationIsClosed of (x := x1))
          }
        case Meet(x1, y1) =>
          proveInU(x1) andThen { s1 =>
            proveInU(y1) andThen2 { s2 =>
              have(premises |- x1 ∈ U /\ y1 ∈ U) by RightAnd(s1, s2)
              have(premises |- (x1 n y1) ∈ U) by Cut(lastStep, lib.meetIsClosed of(x := x1, y := y1))
            }
          }
        case Join(x1, y1) =>
          proveInU(x1) andThen { s1 =>
            proveInU(y1) andThen2 { s2 =>
              have(premises |- x1 ∈ U /\ y1 ∈ U) by RightAnd(s1, s2)
              have(premises |- (x1 u y1) ∈ U) by Cut(lastStep, lib.joinIsClosed of(x := x1, y := y1))
            }
          }
    ...
    * */
    ...
    def proveNoC(using proof: lib.Proof)(gamma: Annotated, delta: Annotated): proof.ProofTacticJudgement = TacticSubproof:
      ...
      (gamma, delta) match
        ...
        case (L(`0`), delta) =>
          have(inU(deltaF) |- 0 <= deltaF) by Weakening(lib.lemmaForP3A of (x := deltaF))
        case (gamma, R(`1`)) =>
          have(inU(gammaF) |- gammaF <= 1) by Weakening(lib.lemmaForP3B of (x := gammaF))
        ...
        case (L(Join(x1, y1)), delta) if proved(L(x1), delta) && proved(L(y1), delta) =>
          ...
          val left12 = s1.bot.left ++ s2.bot.left
          val s3 = have(left12 |- (x1 <= deltaF) /\ (y1 <= deltaF)) by RightAnd(s1, s2)
          have(left12 ++ inU(x1, y1, deltaF) |- (x1 u y1) <= deltaF) by Cut(s3, lib.leftOr of(x := x1, y := y1, z := deltaF))
        ...
        case (gamma, L(x1)) if proved(L(x1), gamma) =>
          ...
          gamma match
            ...
            case R(y1) => // s1: x1 <= y1
              have(s1.bot.left ++ inU(x1, y1) |- !y1 <= !x1) by Cut(s1, lib.commutRL of(x := x1, y := y1))
            case N => // s1: x1 <= 0
              ??? // AR can happen ?
        ...
        // RightAnd
        case (gamma, R(Meet(x1, y1))) if proved(gamma, R(x1)) && proved(gamma, R(y1)) =>
          val s1 = have(prove(gamma, R(x1)))
          val s2 = have(prove(gamma, R(y1)))
          val left12 = s1.bot.left ++ s2.bot.left
          have(left12 |- (gammaF <= x1) /\ (gammaF <= y1)) by RightAnd(s1, s2)
          have(left12 ++ inU(gammaF, x1, y1) |- gammaF <= (x1 n y1)) by Cut(lib.lastStep, lib.rightAnd of(x := gammaF, y := x1, z := y1))
